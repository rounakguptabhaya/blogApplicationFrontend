...
const BlogDetails = () => {
  ...
  const fetchDetails = async () => {
    const res = await axios.get(`https://blog-application-api.herokuapp.com/api/blog/${id}`).catch(err => console.log(err))
  ...
  return (
    ...
    <div> {inputs &&
      <form onSubmit={handleSubmit}>
        <Box border={3} borderColor="'linear-gradient(90deg, rgba(76,5,133,1) 0%, rgba(103,14,109,1) 37%, rgba(214,14,196,1) 100%);'" borderRadius={10} boxShadow="10px 10px 20px #ccc" padding={3} margin={'auto'} marginTop={3} display="flex" flexDirection={'column'} width={'80%'}>
